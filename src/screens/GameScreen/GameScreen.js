...
const GameScreen = ({
    ...
    const [answers, setAnswers] = useState(() => {
        if (isCompleted) {
            if (saveData.success[daysElapsed])
                return saveData.success[daysElapsed];
            if (saveData.failure[daysElapsed])
                return saveData.failure[daysElapsed];
        } else {
        ...
        return extendedAnswers.map((answer, rowIndex) => {
            ...
            if (rowIndex < currentLine || isCompleted) {
                for (let i = 0; i < answerLetters.length; i++) {
                    ...
                    const letter = answerLetters[i].toLowerCase();
                ...
                for (let i = 0; i < answerLetters.length; i++) {
                    const letter =
                        answerLetters[i] && answerLetters[i].toLowerCase();
                    if (letter) {
                        let firstIndex = correctLetters.indexOf(letter);
                        if (firstIndex >= 0) {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            changeScreen({
                type: "scoreboard",
                params: { highlight: answers.length },
            });
        }, 2500);
    ...
    const handleKeyboard = (key) => {
        ...
        if (lowerKey === "enter") {
            ...
            const submittedAnswer =
                answers[answers.length - 1].toLocaleLowerCase();
            ...
            if (allAllowedGuesses.includes(submittedAnswer)) {
                ...
                setTimeout(() => {
                    ...
                    rowRefs.current[currentLine].shake(
                        answers[answers.length - 1].length < currWord.length
                    );
                }, 100);
            }
        ...
        setAnswers((oldValue) => {
            ...
            if (lowerKey === "delete" || lowerKey === "backspace") {
                newAnswers[currentLine] = removeLastChar(
                    newAnswers[currentLine]
                );
                return newAnswers;
            }
    ...
    const keyColours = React.useMemo(() => {
        ...
        for (let i = 0; i < boxValues.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
                ...
                if (
                    letters[value] === "partial" &&
                    row[j].boxState !== "correct"
                ) {
                    continue;
                }
                ...
                <div className={classes.feedbackWrapper}>
                    <div
                        style={{
                            ...
                            top: showFeedback ? "0px" : "15px",
                        }}
                        className={`${classes.feedback} ${
                            negativeFeedback
                                ? classes.negativeFeedback
                                : classes.positiveFeedback
                        }`}
                    >
