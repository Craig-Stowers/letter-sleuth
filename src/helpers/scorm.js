...
_scorm.initScorm = function () {
    ...
    if (!APIOFFLINE) {
        ...
        var completionStatus = _scorm.scormProcessGetValue(
...
_scorm.doExit = function () {
    ...
    if (
        _scorm.reachedEnd === false &&
        window.confirm("Would you like to save your progress to resume later?")
    ) {
        //set exit to suspend
...
_scorm.doUnload = function (pressedExit) {
    ...
    var totalMilliseconds =
        endTimeStamp.getTime() - _scorm.startTimeStamp.getTime();
    var scormTime = _scorm.convertMilliSecondsToSCORMTime(
        totalMilliseconds,
        false
    );
...
_scorm.convertMilliSecondsToSCORMTime = function (
    ...
    intMinutes =
        ((intTotalMilliseconds - intMilliseconds - intSeconds * 1000) / 60000) %
        60;
    intHours =
        (intTotalMilliseconds -
            intMilliseconds -
	...
	*/
    ...
    strCMITimeSpan =
        _scorm.zeroPad(intHours, 4) +
        ":" +
        _scorm.zeroPad(intMinutes, 2) +
        ":" +
        _scorm.zeroPad(intSeconds, 2);
